...
void readyScene(Scene *scene) {
    switch(scene->id) {
        case 0: { // Title screen
            scene->flags[4] = 0;
            initObjectData(scene->data, 0, SPRITE, "./resources/spr_back_0.bmp", 0, 0, 0, 0, 0, SDL_LIGHT_PURPLE,
                           SDL_FLIP_NONE, true);
            initObjectData(scene->data, 1, TEXT, "JEU DE PAVAGE", 260, 24, 0, 4, 0, SDL_WHITE, SDL_FLIP_NONE, true);

            // buttons of main screen
            initObjectData(scene->data, 2, SPRITE, "./resources/spr_btn_main_0.bmp", 336, 200, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, true);
            initObjectData(scene->data, 3, SPRITE, "./resources/spr_btn_main_1.bmp", 336, 256, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, true);
            initObjectData(scene->data, 4, SPRITE, "./resources/spr_btn_main_2.bmp", 336, 312, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, true);

            // how many players buttons
            initObjectData(scene->data, 5, SPRITE, "./resources/spr_btn_play_0.bmp", 336, 202, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
            initObjectData(scene->data, 6, SPRITE, "./resources/spr_btn_play_1.bmp", 336, 258, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);

            // difficulties buttons
            initObjectData(scene->data, 7, SPRITE, "./resources/spr_btn_diff_0.bmp", 336, 202, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
            initObjectData(scene->data, 8, SPRITE, "./resources/spr_btn_diff_1.bmp", 336, 258, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);

            //which size ?
            initObjectData(scene->data, 9, SPRITE, "./resources/spr_btn_size_0.bmp", 336, 202, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
            initObjectData(scene->data, 10, SPRITE, "./resources/spr_btn_size_1.bmp", 336, 258, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
            initObjectData(scene->data, 11, SPRITE, "./resources/spr_btn_size_2.bmp", 336, 314, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);

            // return button
            initObjectData(scene->data, 12, SPRITE, "./resources/spr_btn_undo.bmp", 16, 16, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
        ...
        case 1: { // One Player
            ...
            initObjectData(scene->data, 0, SPRITE, "./resources/spr_back_1.bmp", 0, 0, 0, 0, 0, SDL_PURPLE,
                           SDL_FLIP_NONE, true);

            // Game Grid and labels
            initObjectData(scene->data, 2, GRID, "", x_tmp, y_tmp, scene->flags[7], 3, 0, SDL_BLACK, SDL_FLIP_NONE,
                           true);
            ...
            for (int i = 5; i < 10; i++) {
                initObjectData(scene->data, i, TILE, "", 144 + 104 * (i - 5), 500, 0, 3, 0, SDL_BLACK, SDL_FLIP_NONE,
                               true);
                if (scene->flags[1]) {
            ...
            initObjectData(scene->data, 11, TEXT, "ACT: Selectionnez une tuile", 16, 440, 0, 2, 0, SDL_WHITE,
                           SDL_FLIP_NONE, true);
            initObjectData(scene->data, 12, SPRITE, "./resources/spr_btn_stop.bmp", 64, 16, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, true);
            initObjectData(scene->data, 13, SPRITE, "./resources/spr_btn_save.bmp", 8, 16, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, true);

            // End game Screen
            initObjectData(scene->data, 15, SPRITE, "./resources/spr_back_2.bmp", 160, 96, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
            ...
            initObjectData(scene->data, 20, SPRITE, "./resources/spr_btn_ok.bmp", 336, 376, 0, 0, 0, SDL_WHITE,
                           SDL_FLIP_NONE, false);
        ...
        case 2: { // Two players
            ...
            for (int i = 10; i < 15; i++) {
                initObjectData(scene->data, i, TILE, "", 144 + 104 * (i - 10), 500, 0, 3, 0, SDL_BLACK, SDL_FLIP_NONE,
                               scene->flags[8] ? true : false);
                if (scene->flags[1]) {
                    strcpy(scene->data[i]->sprite, scene->buffers[i - 4]);
                } else {
                    generateTile(scene->data[i]->sprite, flags);
                    ...
                    if (isOnObject(scene, 2, input, 128, 48)) { // New game
                        ...
                        if (fileExists("sav.dat")) {
                            ...
                            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_WARNING, WINDOW_TITLE,
                                                     "Le fichier de sauvegarde est inexistant ou n'a pas pu être ouvert !",
                                                     window->handle);
            ...
            case 1: { // GAME 1-PLAYER
                ...
                switch (scene->flags[4]) {
                        ...
                        if (scene->flags[9] == 0) { // If first tile
                            if (input->mouse[SDL_BUTTON_LEFT]) {
                                ...
                                for (short i = 0; i < size; i++) {
                                    if (isInRect(input, scene->data[2]->x + 2 + 32 * (i % width),
                                                 scene->data[2]->y + 2 + 32 * (i / width), 28, 28)) {
                                        scene->flags[13] = i;
                                        int pos = i - width * (scene->flags[12] / 3) - scene->flags[12] % 3;
                                        if (isInGrid(scene->data[scene->flags[11]]->sprite, pos, scene->flags[12],
                                                     width, height)) {
                                            setTile(scene->data[2]->sprite, scene->data[scene->flags[11]]->sprite, pos,
                                                    -1, width, 0);
                            ...
                            if (input->mouse[SDL_BUTTON_LEFT]) {
                                ...
                                for (short i = 0; i < size; i++) {
                                    if (isdigit(scene->data[2]->sprite[i])
                                        && isInRect(input, scene->data[2]->x + 2 + 32 * (i % width),
                                                    scene->data[2]->y + 2 + 32 * (i / width), 28, 28)) {
                                        scene->flags[13] = i;
                                        int pos = i - width * (scene->flags[12] / 3) - scene->flags[12] % 3;

                                        if (isValidPos(scene->data[2]->sprite, scene->data[scene->flags[11]]->sprite,
                                                       pos, width, false)) {
                                            setTile(scene->data[2]->sprite, scene->data[scene->flags[11]]->sprite, pos,
                                                    scene->flags[12], width, height);
                                            scene->data[2]->flush = true;
                    ...
                    case 3: // End of turn
                        ...
                        bool res = false;
                        for (int i = 5; i < 10; i++) {
                            if (getNumberOfMoves(scene->data[2]->sprite, scene->data[i]->sprite, width, height, false) >
                                0) {
                                res = true;
                            }
                        }
