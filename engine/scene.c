...
void readyScene(Scene *scene) {
    switch(scene->id) {
        case 0: // Title screen
            ...
            scene->data[2] = initObjectData(2, TILE, "XXXXXXXXX", 320, 240, 0, 4, 0, SDL_BLACK, SDL_FLIP_NONE, true);
    ...
    if(!scene->isReady) {
        ...
        switch (scene->id) {
            case 0: // TITLE SCREEN
                ...
                if (isOnButton(scene, 3, input) && input->mouse[SDL_BUTTON_LEFT]) { //si on est le boutton 1 et qu'on clique dessus :
                    scene->data[3]->angle++;  //alors le boutton tourne
                }
                if (isOnButton(scene, 4, input) && input->mouse[SDL_BUTTON_LEFT]) { //si on est le boutton 2 et qu'on clique dessus :
                    scene->data[4]->angle++;  //alors le boutton tourne
                }
                if (isOnButton(scene, 5, input) && input->mouse[SDL_BUTTON_LEFT]) { //si on est le boutton 3 et qu'on clique dessus :
